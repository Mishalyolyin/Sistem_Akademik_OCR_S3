...
class OcrService
{
    ...
    /**
     * Sanity-check a student document (KTP/Ijazah) via OCR. Unlike processImage(),
     * this never rejects/approves anything — it just extracts informational data
     * (NIK, keyword/name matches) for an admin badge.
     *
     * @param string $imagePath
     * @param string $docType 'ktp' or 'ijazah'
     * @param string|null $studentName
     * @return array
     */
    public function processDocument(string $imagePath, string $docType, ?string $studentName = null)
    {
        $scriptPath = base_path('app/Scripts/ocr_processor.py');
        ...
        $command = ['python3', $scriptPath, $imagePath, '--doc-type', $docType];
        ...
        if (!$process->isSuccessful()) {
            Log::channel('ocr')->error('OCR Document Process Failed', [
                'error' => $process->getErrorOutput(),
                'output' => $process->getOutput(),
                'path' => $imagePath,
                'doc_type' => $docType,
        ...
        $output = $process->getOutput();
        Log::channel('ocr')->info('OCR Document Raw Output', ['doc_type' => $docType, 'output' => $output]);

        $decoded = json_decode($output, true);

        if (json_last_error() !== JSON_ERROR_NONE) {
            Log::channel('ocr')->error('OCR Document JSON Decode Error', ['json_error' => json_last_error_msg()]);
            return ['error' => 'Invalid JSON output'];
        }
